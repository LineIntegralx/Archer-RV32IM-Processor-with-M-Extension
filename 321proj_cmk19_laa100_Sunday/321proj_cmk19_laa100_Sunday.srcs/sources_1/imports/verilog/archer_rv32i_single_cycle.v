...
module archer_rv32i_single_cycle (
	...
	output dmem_wen,
	output [3:0] dmem_ben,
	output Ovr_Flw_Flag, //added outputs for the Overflow flag
	output Zero_Flag, //added outputs for the Zero flag
	output [`XLEN-1:0] M_output 
	);
    ...
    //wires added by Claritta & Lynn
    wire LC_enable;
    wire [`CONTROLOPS_SIZE-1:0] LC_M_Inst_picker;
    //M_Inst wires
    wire [`XLEN-1:0] d_M_Inst_out; //result of M_Inst
    wire [`XLEN-1:0] d_final_res; //final result
    ...
    control control_inst (.instruction(d_instr_word), 
    			  ...
    			  .CONTROLop(LC_M_Inst_picker),
    			  .ALUorCONTROL(LC_enable));
    ...
    alu alu_inst (.inputA(d_alu_src1),
   	 ...
   	 M_Inst M_Inst (.rs1(d_alu_src1),
                   .rs2(d_alu_src2),
                   .ALUorCONTROL(LC_enable),
                   .CONTROLop(LC_M_Inst_picker),
                   .rd(d_M_Inst_out),
                   .Ovr_Flw(Ovr_Flw_Flag),
                   .Zero(Zero_Flag));	
    			
    mux2to1 result_mux(.sel(LC_enable),
                       .input0(d_alu_out),
                       .input1(d_M_Inst_out),
                       .muxout(d_final_res));
